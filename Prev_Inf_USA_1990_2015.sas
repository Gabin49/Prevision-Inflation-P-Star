...
DATA memoire.USA_ALL;
    merge memoire.USA_M2
          ...
          memoire.USA_RGDP;
    by observation_date;
    where observation_date >= '01JAN1990'd and observation_date < '01JAN2019'd;
RUN;
...
PROC SGPLOT data=memoire.USA_ALL_Ps;
    ...
    title "Evolution of observed p and p*
";
RUN;
...
PROC SGPLOT data=memoire.pred_ecm;
    ...
    xaxis label="Date";
    yaxis label="dif_p";
    title "Comparison between observed and predicted dif\_p (in-sample)
";
RUN;
...
DATA estimation prediction;
    set memoire.USA_ECM;
    if observation_date <= '01JAN2015'd then output estimation;
    else if observation_date > '01JAN2015'd then output prediction;
RUN;
...
PROC MEANS data=estimation noprint;
    where observation_date >= '01JAN2015'd;
    var dif_p;
...
PROC SGPLOT data=memoire.USA_ALL_Ps;
    series x=observation_date y=ecart_p / lineattrs=(color=red) legendlabel="p - p*";
